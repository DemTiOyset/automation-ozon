...
from application.repositories.market.client import get_order
from application.repositories.repo import OrderRepository
from application.repositories.responses import Responses
from application.repositories.sheets import create_order_items_in_sheets
from application.repositories.transformation import _transforming_order_data_creation, _dto_to_order
from application.shemas.notification import OrderCreatedNotificationDTO, OrderUpdatedShipmentDateNotificationDTO, \
...
async def handle_order_created(
    ...
    try:
        ...
        if len(orders_from_db) == 0:    # Проверка на наличие записи о заказе в бд, если записей нет, то программа записывает
            order_items_dto = await _transforming_order_data_creation(
            ...
            orders = await OrderRepository.create_order_with_items(dto_to_model)   # Создание записи в бд
        ...
        order_items_to_sheet = await create_order_items_in_sheets(repo, order_items_dto)   # Создание записи о заказе в таблице
...
async def handle_order_updated_shipment_date(
    ...
    try:
        ...
        order = await OrderRepository.get_first_order_by_posting_number(notification.posting_number)
        ...
        orders = await OrderRepository.update_order_shipment_date(notification.posting_number, notification.new_cutoff_date)
...
async def handle_order_updated_status(
    ...
    try:
        ...
        if last_event_time_from_db >= notification.changed_state_date:
            return Responses.responses(
                {"message": "The changed state date is less than the last change date in the database"}
            )

        orders = await OrderRepository.update_order_status(notification.posting_number, notification.new_state)
